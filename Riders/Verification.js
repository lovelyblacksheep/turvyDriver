   ...
   const [verification, setVerification] = React.useState(verificationId);   
    ...
    useEffect ( ()=>{
   	firebase.auth().onAuthStateChanged((user) => {	
   		console.log("STATUS user 2  ",code);
       //setValue(code)
   		//alert(user);
  	...
  	const sendData = async() => {
        ...
        //remove all if code when remove dummy user
        //otherwise remove only if condition
        if(phone == dummyPhone){
    ...
    <SafeAreaView style={stylesv.root} keyboardShouldPersistTaps='handled'>
     <FlashMessage position="top" style={{marginTop:StatusBarheight,borderRadius:2}}  />
      ...
      <Text style={stylesv.heading}>We sent OTP code to Verify your number</Text>
      	...
      	<View style={{borderBottomColor:'#ccc',borderBottomWidth:1, width: 250,height:55,alignContent:'center',alignItems:'center',marginTop:8,marginBottom:8}}>

...
import { View, ScrollView, StyleSheet, ImageBackground, TouchableHighlight, TouchableWithoutFeedback, TouchableOpacity, Image, TextInput, ActivityIndicator } from 'react-native';
import { Picker } from '@react-native-picker/picker';
import { LinearGradient } from 'expo-linear-gradient';
import { styles, DOMAIN, dummyPhone } from './Constant'
...
import { sendoptreg } from './api/sendoptreg';
...
const Login = ({ props }) => {
	...
	const sendopt = async () => {
		...
		if (mobileNumber == dummyPhone) {
			return navigation.replace('Verification', { phone: mobileNumber, countrycode: countryPick, fromwhere: 'login', verificationId: '', code: '111111' });
		}
		//alert(phoneNumber);
		try {
		...
		try {
			...
			fetch(DOMAIN + 'api/rider/login/phone', {
				...
				if (result.status == 1) {
					...
					sendoptreg(countryPick, mobileNumber, navigation, flref).finally(() => setSpinner(false));
		...
		return (<><View style={styles.container}>
			...
			<View style={{ flex: 1, position: 'absolute', width: '95%', top: 0, bottom: 0, left: 10, right: 0, paddingBottom: 50 }}>
				...
				<ScrollView keyboardShouldPersistTaps='handled'>
					...
					<View style={{ paddingLeft: 30, paddingRight: 30 }}>
						...
						<View style={{ marginLeft: 20, marginTop: 10, alignContent: 'center' }}>
							<View >
								<View>
									{codesent ?
										(<><ActivityIndicator size="large" color="#74c0ee" />
											<Text style={{ alignContent: 'center', textAlign: 'center', color: '#006400' }}>Verification Code Send ....</Text></>)
										:
										(<></>)
									}
								</View>
			...
			<View style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', backgroundColor: 'rgba(255, 255, 255, 0.8)' }}>
				<View style={{ justifyContent: 'center', alignItems: 'center', alignItems: 'center', flex: 1, padding: 15 }}>
					...
					<Text style={{ fontSize: 15, fontFamily: "Metropoli	s-Regular" }}>Terms of service <TouchableWithoutFeedback onPress={() => navigation.navigate('PrivacyPolicy')}><Text style={[styles.themetextcolor, styles.strong]}>PRIVACY POLICY</Text></TouchableWithoutFeedback> </Text>
				</View>
			</View>

 ...
 UNSAFE_componentWillUnmount() {
	...
	async getDriverRating(){
		await AsyncStorage.getItem('accesstoken').then((value) => {
      	...
      	fetch('https://www.turvy.net/api/rider/driverRating/'+this.state.bookingdriver.id,{
     	  	method: 'GET', 
		   headers: new Headers({
		     'Authorization': 'Bearer '+value, 
		     'Content-Type': 'application/json'
   			...
   			<Col size={6}>
   				<View style={{alignContent:'center',justifyContent:'center',flex:1,flexDirection:'row'}}>
   					<Row style={{flex:1,alignItems:'center',justifyContent:'center'}}>
						<Col style={{width:60}}>
						{this.state.driveravatar && this.state.driveravatar != null ? 
							(<Image 
						...
						<Col>
							<Text style={{fontSize:12,color:'gray'}}>Distance Travel</Text>
							<Text style={{fontSize:12,fontWeight:'bold'}}>{this.state.distancetravel} km</Text>
						</Col>
					</Row>
				</View>
			</Col>
		...
		<Row style={{height:40}}>
   		...
   		<Row style={{height:50}}>
			...
			<Row size={12}>
				...
				<Col size={2}><Text style={{ fontSize:13, color:'grey'}}>{this.state.start_time}</Text></Col>
			</Row>
		</Row>
	 ...
	 :null
		...
		<Row style={{height:40,marginLeft:-2,marginTop:-15}}>
			...
			<Row  size={12}>
				...
				<Col size={2} >
					<Text style={{ fontSize:13, color:'grey', paddingTop:10}} >{this.state.end_time}
				</Text>
				</Col>
			</Row>
		</Row>
				...
				{/* <Row style={{height:70}}>
   				...
   				<Row>
					<Col>
						...
						>
							<Text style={{color:'#FFF',fontSize:18}}>Rate Driver</Text>
						</TouchableOpacity>
					</Col>
				</Row>
					</Col>
